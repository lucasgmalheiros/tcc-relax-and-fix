...
def extract_features(problem_data):
    ...
    instance_features = {
        ...
        'p25_capacity': np.percentile(cap.flatten(), 25),
        'p50_capacity': np.percentile(cap.flatten(), 50),
        'p75_capacity': np.percentile(cap.flatten(), 75),
        'iqr_capacity': np.percentile(cap.flatten(), 75) - np.percentile(cap.flatten(), 25),

        # PRODUCTION TIME
        'total_production_time': np.sum(b),
        'min_production_time': np.min(b),
        'avg_production_time': np.mean(b),
        'max_production_time': np.max(b),
        'std_production_time': np.std(b),
        'skew_production_time': skew(b.flatten()),
        'kurt_production_time': kurtosis(b.flatten()),
        'cv_production_time': np.std(b.flatten()) / np.mean(b.flatten()) if np.mean(b.flatten()) != 0 else 0,
        'p25_production_time': np.percentile(b.flatten(), 25),
        'p50_production_time': np.percentile(b.flatten(), 50),
        'p75_production_time': np.percentile(b.flatten(), 75),
        'iqr_production_time': np.percentile(b.flatten(), 75) - np.percentile(b.flatten(), 25),

        # SETUP TIME
        'total_setup_time': np.sum(f),
        'min_setup_time': np.min(f),
        'avg_setup_time': np.mean(f),
        'max_setup_time': np.max(f),
        'std_setup_time': np.std(f),
        'skew_setup_time': skew(f.flatten()),
        'kurt_setup_time': kurtosis(f.flatten()),
        'cv_setup_time': np.std(f.flatten()) / np.mean(f.flatten()) if np.mean(f.flatten()) != 0 else 0,
        'p25_setup_time': np.percentile(f.flatten(), 25),
        'p50_setup_time': np.percentile(f.flatten(), 50),
        'p75_setup_time': np.percentile(f.flatten(), 75),
        'iqr_setup_time': np.percentile(f.flatten(), 75) - np.percentile(f.flatten(), 25),

        # PRODUCTION COSTS
        'total_production_cost': np.sum(c),
        'min_production_cost': np.min(c),
        ...
        'cv_production_cost': np.std(c.flatten()) / np.mean(c.flatten()) if np.mean(c.flatten()) != 0 else 0,
        'p25_production_cost': np.percentile(c.flatten(), 25),
        'p50_production_cost': np.percentile(c.flatten(), 50),
        'p75_production_cost': np.percentile(c.flatten(), 75),
        'iqr_production_cost': np.percentile(c.flatten(), 75) - np.percentile(c.flatten(), 25),

        # SETUP COSTS
        'total_setup_cost': np.sum(s),
        'min_setup_cost': np.min(s),
        ...
        'cv_setup_cost': np.std(s.flatten()) / np.mean(s.flatten()) if np.mean(s.flatten()) != 0 else 0,
        'p25_setup_cost': np.percentile(s.flatten(), 25),
        'p50_setup_cost': np.percentile(s.flatten(), 50),
        'p75_setup_cost': np.percentile(s.flatten(), 75),
        'iqr_setup_cost': np.percentile(s.flatten(), 75) - np.percentile(s.flatten(), 25),

        # TRANSPORTATION COSTS
        'total_transportation_cost': np.sum(r),
        'min_transportation_cost': np.min(r),
        'avg_transportation_cost': np.mean(r),
        'max_transportation_cost': np.max(r),
        'std_transportation_cost': np.std(r),
        'skew_transportation_cost': skew(r.flatten()),
        'kurt_transportation_cost': kurtosis(r.flatten()),
        'cv_transportation_cost': np.std(r.flatten()) / np.mean(r.flatten()) if np.mean(r.flatten()) != 0 else 0,
        'p25_transportation_cost': np.percentile(r.flatten(), 25),
        'p50_transportation_cost': np.percentile(r.flatten(), 50),
        'p75_transportation_cost': np.percentile(r.flatten(), 75),
        'iqr_transportation_cost': np.percentile(r.flatten(), 75) - np.percentile(r.flatten(), 25),

        # INVENTORY COSTS
        'total_inventory_cost': np.sum(h),
        'min_inventory_cost': np.min(h),
        'avg_inventory_cost': np.mean(h),
        'max_inventory_cost': np.max(h),
        'std_inventory_cost': np.std(h),
        'skew_inventory_cost': skew(h.flatten()),
        'kurt_inventory_cost': kurtosis(h.flatten()),
        'cv_inventory_cost': np.std(h.flatten()) / np.mean(h.flatten()) if np.mean(h.flatten()) != 0 else 0,
        'p25_inventory_cost': np.percentile(h.flatten(), 25),
        'p50_inventory_cost': np.percentile(h.flatten(), 50),
        'p75_inventory_cost': np.percentile(h.flatten(), 75),
        'iqr_inventory_cost': np.percentile(h.flatten(), 75) - np.percentile(h.flatten(), 25),

        # UTILIZATION
        'total_utilization': np.sum(utilization),
        ...
        'cv_utilization': np.std(utilization) / np.mean(utilization) if np.mean(utilization) != 0 else 0,
        'p25_utilization': np.percentile(utilization, 25),
        'p50_utilization': np.percentile(utilization, 50),
        'p75_utilization': np.percentile(utilization, 75),
        'iqr_utilization': np.percentile(utilization, 75) - np.percentile(utilization, 25),
    }
