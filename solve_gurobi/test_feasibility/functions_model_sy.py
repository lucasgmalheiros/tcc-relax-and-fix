...
def model_sy(folder_path, instancia):
    ...
    Q = m.addVars(I, J, T, vtype=GRB.CONTINUOUS, name='Q', lb=0)
    # # Quantidade transportada (i, j, k(um outro j), t)  FIX TO 0 -> FEASIBILITY WITHOUT TRANSPORTATION
    W = m.addVars(I, J, J, T, vtype=GRB.CONTINUOUS, name='W', lb=0, ub=0)
...
def main_solve(folder_path, results_file, time_limit):
    for file in lista_instancias(folder_path, results_file):
        ...
        m.Params.SolutionLimit = 1  # Parada quando acha solução factível
